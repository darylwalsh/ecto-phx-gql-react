defmodule Reactolatry.Vacation.Booking do
  use Ecto.Schema
  ...
  defp dates_available?(start_date, end_date, place_id) do
    query =
      from booking in Reactolatry.Vacation.Booking,
        where:
    ...
    case Reactolatry.Repo.all(query) do
      [] -> true
      _ -> false
    end
  end
  ...
  defp calculate_total_price(changeset) do
    case changeset.valid? do
      true ->
        ...
        place = Reactolatry.Repo.get!(Reactolatry.Vacation.Place, place_id)
